...
namespace GestuerRecognition
{
    ...
    /// </summary>
    public sealed class PointCloudRecognizer
    {
        /// <summary>
        ...
        /// <returns></returns>
        public static SpellName Classify(Gesture candidate, Gesture[] trainingSet)
        {
            float minDistance = float.MaxValue;
            SpellName gestureClass = 0;
            foreach (Gesture template in trainingSet)
            {
                ...
                {
                    Debug.Log(dist);
                    if (dist > 0.20f) continue;
                    minDistance = dist;
                    gestureClass = template.Name;
                    Debug.Log(minDistance);
                }
            }
            Debug.Log(gestureClass.ToString());
            return gestureClass;
        }
        ...
        private static float CloudDistance(Point[] points1, Point[] points2, int startIndex)
        {
            ...
            do
            {
                ...
                for (int j = 0; j < n; j++)
                    if (!matched[j])
                    {
                        float dist = Geometry.SqrDistance(points1[i], points2[j]);  // use squared Euclidean distance to save some processing time
                        if (dist < minDistance)
                        {
                            minDistance = dist;
                            index = j;
